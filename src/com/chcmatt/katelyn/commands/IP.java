...
@Command(name="ip", desc="Gets information about the specified IP address.", adminOnly=false)
public class IP extends GenericCommand
{
	...
	public void execute()
	{
		if (event.hasNoArgs())
		{
			event.respond(Colors.setBold("This command requires arguments.") + " Syntax: ip <IP Address>" );
		}
		else
		{
			String address = event.getCommandArgs().get(0);
			...
			{
				event.respond(event.getUser().getNick() + ": " + Colors.setColor("IP: ", Colors.DARK_BLUE + Colors.BOLD) + ip + " " +
